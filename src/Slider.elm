module Slider exposing (Msg, Slider, init, slider, update)

import Element exposing (Element, behindContent, centerY, el, fill, fillPortion, focused, height, htmlAttribute, mouseOver, padding, paddingXY, px, rgb, rgba, row, scale, spacing, text, width)
        ...
        , Element.Input.slider
            ...
            , behindContent <|
                row
                    ...
                    [ el
                        [ width <| fillPortion <| round ((value.value - min) / (max - min) * 100)
                        , height fill
                        , Background.color <| rgb 0 0 0
                        ]
                        Element.none
                    , el
                        [ width <| fillPortion <| round ((max - value.value) / (max - min) * 100)
                        , height fill
